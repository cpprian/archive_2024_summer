...
size_t heap_get_largest_used_block_size(void) {
    ...
    while (chunk != NULL) {
        if (chunk->free == 0 && chunk->size > max) {
            max = chunk->size;
        }
        chunk = chunk->next;
    }
...
enum pointer_type_t get_pointer_type(const void* const pointer) {
    ...
    if ((char*)memory_manager.memory_start - (char*)pointer > 0 ||
        (char*)memory_manager.memory_start + memory_manager.memory_size - (char*)pointer < 0)
    {
...
void merge_chunks(struct memory_chunk_t* chunk) {
    if (chunk->next != NULL && chunk->next->free == 1) {
        ...
        if (chunk->next != NULL) {
            chunk->next->prev = chunk;
            // chunk->next->checksum = sum_control_block(chunk->next);
            checksum_all_chunks();
        }
        // chunk->checksum = sum_control_block(chunk);
        checksum_all_chunks();
    ...
    if (chunk->prev != NULL && chunk->prev->free == 1) {
        ...
        // chunk->prev->checksum = sum_control_block(chunk->prev);
        checksum_all_chunks();
    }
}
